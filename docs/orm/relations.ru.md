...
`add()` остаётся идемпотентным и при конкурентном доступе: если два вызова
гонятся за одной и той же парой, вставка проигравшего упирается в составной
первичный ключ through-таблицы и трактуется как «уже связаны», а не бросается
наружу.
...
- **Имя обратной связи** (`related_name`, или `<model>_set`, если он не
  задан) — **один** дополнительный запрос (`fk__in=[...]` к *исходной*
  модели, сгруппированный по id fk на стороне Python):

```python
for author in Author.objects.prefetch_related("books"):
    author.books.all()      # из кэша — без запроса на каждого автора
    list(author.books)      # обычная итерация тоже из кэша
    author.books.filter(title__startswith="A")  # а вот здесь снова запрос
```

  Из кэша читает только голый `.all()` (или прямая итерация по связи, без
  дополнительных условий) — всё, что дописано после (`.filter()`,
  `.exclude()`, `.order_by()`, ...), это уже другой запрос и всегда идёт в
  базу, точно как у обычного QuerySet. `.count()` на обратной связи тоже
  всегда запрашивает заново (кэш prefetch он не смотрит — как и `.count()`
  у любого другого QuerySet) — если узкое место именно в подсчёте на
  каждой строке, `annotate(n=Count("books"))` избегает этого (см.
  [Запросы](queries.md)).
