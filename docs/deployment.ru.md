...
- `ENDOCORE_DEV=0` — отключить dev-вотчер в процессе (заодно выключает
  `/docs` + `/openapi.json`, поскольку по умолчанию они включены только в dev).
- `ENDOCORE_DEFAULT_VERSION=latest` — резолвить пути без версии.
- `ENDOCORE_OPENAPI=1` — отдавать `/docs` + `/openapi.json` даже при
  `ENDOCORE_DEV=0`. По умолчанию в продакшене выключено намеренно —
...
- [ ] `ENDOCORE_DEV=0` (без вотчера; заодно выключает `/docs` по умолчанию).
